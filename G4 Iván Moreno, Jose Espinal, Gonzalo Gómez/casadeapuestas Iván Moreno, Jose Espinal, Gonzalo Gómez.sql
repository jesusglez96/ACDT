...
*/
CREATE OR ALTER PROCEDURE RealizarApuestaTipo1
	@cantidad AS smallmoney,
	@cuota AS decimal(5,2),
	@IDPartido AS uniqueidentifier,
	@CorreoUsuario AS varchar(45),
...
GO
CREATE OR ALTER TRIGGER LimiteApuestaTipo1 ON ApuestasTipo1 AFTER INSERT AS
BEGIN
...
GO
CREATE or ALTER TRIGGER LimiteApuestaTipo2 ON ApuestasTipo2 AFTER INSERT AS
BEGIN

	DECLARE @IDPartido AS uniqueidentifier
	DECLARE @IDApuesta AS uniqueidentifier
	DECLARE @Goles AS tinyint
	DECLARE @esLocaloVisitante AS bit
	...
	INTO @IDPartido, @IDApuesta, @Goles, @esLocaloVisitante
	...
	END
	--En este tipo de apuesta solo hacen falta los goles de un equipo
	BEGIN
		RAISERROR('Limite alcanzado, las apuestas estan cerradas',16,1)
		ROLLBACK
	END
...
GO
CREATE or ALTER TRIGGER LimiteApuestaTipo3 ON ApuestasTipo3 AFTER INSERT AS
BEGIN
	...
	INTO @IDPartido, @IDApuesta, @quiniela
...
GO
CREATE or ALTER TRIGGER comprobarSaldo ON Apuestas AFTER INSERT AS
BEGIN
